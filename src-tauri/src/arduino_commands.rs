...
/// Uses winget on Windows, brew on macOS, or direct download on Linux
#[command]
pub async fn arduino_install_cli() -> Result<ArduinoResult, String> {
    #[cfg(target_os = "windows")]
    {
        // Try winget first (Windows 10/11)
        let winget_result = create_hidden_command("winget")
            .args(&["install", "ArduinoSA.CLI", "--accept-source-agreements", "--accept-package-agreements"])
            .output();
        ...
        match winget_result {
            Ok(output) => {
                ...
                if output.status.success() || stdout.contains("successfully installed") {
                    return Ok(ArduinoResult {
                        success: true,
                        stdout: "Arduino CLI installed successfully via winget. Please restart the IDE.".to_string(),
                        stderr: String::new(),
                        exit_code: 0,
                    });
                }
                ...
                let choco_result = create_hidden_command("choco")
                    .args(&["install", "arduino-cli", "-y"])
                    .output();
                
                match choco_result {
                    Ok(choco_output) => {
                        if choco_output.status.success() {
                            return Ok(ArduinoResult {
                                success: true,
                                stdout: "Arduino CLI installed successfully via Chocolatey. Please restart the IDE.".to_string(),
                                stderr: String::new(),
                                exit_code: 0,
                            });
                        }
                ...
                // Return instructions for manual install
                return Ok(ArduinoResult {
                    success: false,
                    stdout: String::new(),
                    stderr: format!(
                        "Auto-install failed. Please install manually:\n\
                        1. winget install ArduinoSA.CLI\n\
                        2. Or download from: https://arduino.github.io/arduino-cli/latest/installation/\n\
                        winget error: {} {}",
                        stdout, stderr
                    ),
                    exit_code: 1,
            ...
            Err(e) => {
                return Ok(ArduinoResult {
                    ...
                    stdout: String::new(),
                    stderr: format!(
                        "Package manager not found. Please install manually:\n\
                        1. Install winget (Windows Package Manager)\n\
                        2. Run: winget install ArduinoSA.CLI\n\
                        3. Or download from: https://arduino.github.io/arduino-cli/latest/installation/\n\
                        Error: {}",
                        e
                    ),
                    exit_code: 1,
    ...
    {
        // Try Homebrew
        let brew_result = create_hidden_command("brew")
        ...
        match brew_result {
            Ok(output) => {
                if output.status.success() {
                    return Ok(ArduinoResult {
                        success: true,
                        stdout: "Arduino CLI installed successfully via Homebrew. Please restart the IDE.".to_string(),
                        stderr: String::new(),
                        exit_code: 0,
                    });
                }
                ...
                return Ok(ArduinoResult {
                    success: false,
                    stdout: String::new(),
                    stderr: format!("Homebrew install failed: {}", stderr),
                    exit_code: 1,
                });
            }
            Err(_) => {
                return Ok(ArduinoResult {
                    success: false,
                    stdout: String::new(),
                    stderr: "Homebrew not found. Please install: brew install arduino-cli".to_string(),
                    exit_code: 1,
                });
            }
        }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let result = create_hidden_command("sh")
            .arg("-c")
            .arg(script)
            .output();
        ...
        match result {
            Ok(output) => {
                if output.status.success() {
                    return Ok(ArduinoResult {
                        success: true,
                        stdout: "Arduino CLI installed successfully. Please restart the IDE and ensure ~/bin is in your PATH.".to_string(),
                        stderr: String::new(),
                        exit_code: 0,
                    });
                }
                ...
                return Ok(ArduinoResult {
                    success: false,
                    stdout: String::new(),
                    stderr: format!("Install script failed: {}", stderr),
                    exit_code: 1,
                });
            }
            Err(e) => {
                return Ok(ArduinoResult {
                    success: false,
                    stdout: String::new(),
                    stderr: format!("Failed to run install script: {}", e),
                    exit_code: 1,
                });
            }
        }
    ...
    #[cfg(not(any(target_os = "windows", target_os = "macos", target_os = "linux")))]
    {
        return Ok(ArduinoResult {
            success: false,
            stdout: String::new(),
            stderr: "Auto-install not supported on this platform. Please install Arduino CLI manually.".to_string(),
            exit_code: 1,
        });
    }
}
...
pub async fn arduino_compile(
    ...
    // Use "." as sketch path and set working directory to the sketch folder
    // This is more reliable than passing full paths with quotes
    let mut cmd = format!(
    ...
    // Execute from the sketch directory
    let result = execute_arduino_command(&cmd, Some(sketch_path)).await?;
...
pub async fn arduino_preprocess(
    ...
    execute_arduino_command(&cmd, Some(sketch_path)).await
}
...
pub async fn arduino_install_core(core: String) -> Result<ArduinoResult, String> {
    // Don't add quotes - the core name like "arduino:avr" doesn't need them
...
pub async fn arduino_serial_monitor_start(
    ...
    // Check if monitor exists and stop it first
    let needs_stop = {
    ...
    // Open port BEFORE spawning thread (avoids race condition)
    ...
    // Control DTR to handle Arduino reset properly
    ...
    // Flush bootloader garbage (null bytes)
    let mut flush_buf = [0u8; 4096];
    loop {
        match serial_port.read(&mut flush_buf) {
            Ok(_) => continue,
            Err(_) => break,
        }
    }

    // Clone port for writing (separate handle, same underlying port)
    let write_port = serial_port.try_clone()
        .map_err(|e| format!("Cannot clone port for writing: {}", e))?;
    let writer: Arc<Mutex<Box<dyn std::io::Write + Send>>> =
        Arc::new(Mutex::new(Box::new(write_port)));
    ...
    // Spawn reader thread - port is moved into thread
    let thread_handle = thread::spawn(move || {
        let mut buffer = [0u8; 2048];
        let mut batch_buf: Vec<u8> = Vec::with_capacity(4096);
        let mut last_emit = std::time::Instant::now();
        let batch_interval = Duration::from_millis(16); // ~60fps batching
        ...
        while *running_clone.lock().unwrap() {
            match serial_port.read(&mut buffer) {
                Ok(bytes_read) if bytes_read > 0 => {
                    batch_buf.extend_from_slice(&buffer[..bytes_read]);

                    // Batch: emit at ~60fps OR when buffer exceeds 4KB
                    if last_emit.elapsed() >= batch_interval || batch_buf.len() >= 4096 {
                        let data = String::from_utf8_lossy(&batch_buf).to_string();
                ...
                Err(ref e) if e.kind() == std::io::ErrorKind::TimedOut => {
                    // Flush remaining buffer on timeout (device paused sending)
            ...
            // NO thread::sleep here! 50ms serial timeout handles idle waiting.
        }
    ...
    // Store handle with writer for send function
...
/// Send data through serial monitor (uses existing open connection)
#[command]
pub async fn arduino_serial_monitor_send(
    ...
    // Use the writer from the already-open monitor connection
    // instead of opening a new port (which conflicts and uses wrong baud)
    let monitors = SERIAL_MONITORS.lock().map_err(|e| e.to_string())?;

    if let Some(handle) = monitors.get(&port) {
        if let Some(ref writer_arc) = handle.writer {
            let mut writer = writer_arc.lock().map_err(|e| e.to_string())?;
            let mut send_data = data;
            if add_newline.unwrap_or(true) {
                send_data.push('\n');
            }
            writer.write_all(send_data.as_bytes())
                .map_err(|e| format!("Failed to write: {}", e))?;
            let _ = writer.flush();
            Ok(())
        } else {
            Err("Serial port writer not available".to_string())
        }
    } else {
        Err(format!("No monitor running on port {}. Start the monitor first.", port))
    }
}
