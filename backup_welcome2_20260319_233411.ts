...
function initializeTabBadges(): void {
  console.log('🏷️ Initializing tab badge system...');
 ...
 try {
    ...
    document.addEventListener('file-tree-refresh', (event: any) => {
      ...
      if (action === 'delete' && path) {
        ...
        // ============================================================================
        console.log('🗑️ Removing deleted file from tree:', path);
        ...
        if (fileName && filePath && fileExplorer?.addFile) {
          ...
          console.warn('⚠️ Cannot add file to tree');
  ...
  import('./fileOperations/treeGenerator').then(treeModule => {
    ...
    import('./ide/fileExplorer/fileClickHandlers').then(handlerModule => {
      if (handlerModule.setupContextMenu) {
        ...
        console.warn('⚠️ setupContextMenu not found in fileClickHandlers');
      }
    }).catch(err => {
...
async function initializeGit(): Promise<void> {
  ...
  try {
    ...
    if (!isGitInstalled) {
      console.warn('⚠️ Git not installed. Git features disabled.');
      return;
    }
    ...
    if (projectPath) {
      ...
      if (isGitRepo) {
        ...
        try {
          ...
          console.warn('⚠️ [Git] Could not get status:', e);
        }
      }
    }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.ctrlKey && e.shiftKey && e.key.toUpperCase() === 'G') {
      ...
      if (projectPath) {
        ...
        console.warn('⚠️ No project path - open a folder first');
      }
    }
  ...
  aiGenerateBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (addedLines === 0 && removedLines === 0) {
        diffAnalysis += '\n\n⚠️ Note: Could not extract line-by-line changes. Files are marked as modified but diff content unavailable.';
...
async function init(): Promise<void> {
  try {
    ...
    if (isInitialized) {
      console.log('⚠️ Already initialized, cleaning up duplicates only...');
      cleanupDuplicates();
  return;
}
...
// // initializeProjectFolderContextMenu(); // REMOVED: duplicate call // REMOVED: duplicate
  ...
  setTimeout(function() {
    ...
    try {
      ...
      console.warn('⚠️ Message UI Fix skipped:', e);
    }
   ...
   // initializePdfHandler();//  initializeAITerminalIntegration();
    ...
    setTimeout(() => setupGitMenu(), 500);
     ...
     initializeEditorContextIntegration();
initMessageCollapse();
// 🗑️ REMOVE: Auto Mode button from terminal header (not needed)
setTimeout(() => {
...
function removeAutoModeButton(): void {
  ...
  if (autoModeBtn) {
    autoModeBtn.remove();
    console.log('🗑️ Removed Auto Mode button (#autonomous-mode-toggle)');
  }
  ...
  document.querySelectorAll('.terminal-action[title*="Auto Mode"], .terminal-action[title*="Auto Apply"]').forEach(btn => {
    btn.remove();
    console.log('🗑️ Removed Auto Mode button (by title)');
  });
...
function injectTerminalWelcomeUI(): void {
  ...
  el.innerHTML = '<div style="font-family:Consolas,Monaco,"Courier New",monospace;font-size:11px;line-height:1.55;color:#3a7a3a;padding:4px 6px 4px 8px;border-left:2px solid #1e5c1e">' +'<span style="color:#4ec94e;font-weight:700;letter-spacing:.5px">&#9889; OPERATOR X02</span><br>' +'<span style="color:#2d5a2d;font-size:10px">' + new Date().toLocaleDateString('en-GB',{weekday:'short',day:'2-digit',month:'short',year:'numeric'}) + '  ' + new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true}).toLowerCase() + '</span>' +'<div style="border-top:1px solid #1a3a1a;margin:2px 0"></div>' +'<span style="color:#3a7a3a">run<span style="color:#1e3e1e">...........</span><span style="color:#4ec94e">Ctrl+Shift+B</span></span><br>' +'<span style="color:#3a7a3a">chat<span style="color:#1e3e1e">.........</span><span style="color:#4ec94e">Ctrl+Shift+A</span></span><br>' +'<span style="color:#3a7a3a">surgical<span style="color:#1e3e1e">.....</span><span style="color:#4ec94e">active</span></span><br>' +'<span style="color:#3a7a3a">gpu<span style="color:#1e3e1e">............</span><span style="color:#4ec94e">live</span></span>' +'<div style="border-top:1px solid #1a3a1a;margin:2px 0"></div>' +'<span style="color:#2a4a2a;font-style:italic;font-size:10px">Coding is Art. Feel it. Enjoy it.</span>' +'</div>';
        ...
        (items || []).map(async (item: any) => {
          if (item.is_directory) {
            try {
              ...
              console.warn(`⚠️ [Persist] Could not read children of ${item.path}:`, err);
              return { ...item, children: [] };
            }
          }
          return item;
...
document.addEventListener('project-closed', () => {
  ...
  // 🛡️ Guard: Don't clear during restore
  if ((window as any).__isRestoringProject) {
    console.log('🛡️ [Event] project-closed BLOCKED during restore');
    return;
  }
  localStorage.removeItem('ide_last_project_path');
  localStorage.removeItem('currentProjectPath');
...
setTimeout(() => {
  import('./ide/projectCreation/ui/modernModal').then(module => {
    ...
    console.log('ℹ️ Project menu dropdown handled by setupProjectMenu()');
  }).catch(err => {
...
try {
  ...
  try {
    ...
    console.warn('⚠️ Orchestrator UI not available:', e);
  }
...
if (import.meta.env.DEV) {
  ...
  setTimeout(() => {
    try {
      ...
      console.log('ℹ️ Intelligent Assistant UI disabled (using unified-status-bar instead)');
    } catch (uiError) {
   ...
   function initializeBuildSystemDropdown() {
     ...
     const checkInterval = setInterval(() => {
       ...
       if (runButtonExists) {
         ...
         if (existingDropdown) {
           console.log('⚠️  Dropdown already exists, skipping');
           return;
         }
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (savedFullConfig) {
      ...
      if (savedKey && savedKey.length > 5) {
        ...
        console.log(`⚠️ [CHAIN] No API key for ${segment.provider}`);
      }
    }
...
`;
      ...
      const projPath = (window as any).currentProjectPath || 
                      ...
                      localStorage.getItem('lastProjectPath');
      if (projPath) context += `🏗️ Current Folder: ${projPath}\n`;
    } catch (e) { 
...
try {
  ...
  if (aiFileExplorer && !fullMessage.includes('[🔍 AI File Explorer')) {
    ...
    if (aiSearchEnabled) {
      ...
      if (isAskingAboutOpenCode && !isAskingAboutProject) {
        console.log('🖥️ User asking about OPEN CODE - including editor content');
        ...
        if (editor) {
          ...
          console.log('⚠️ No editor content available');
        }
      ...
      // ============================================
        ...
        if (chatContainer) {
          ...
          console.warn('⚠️ Chat container not found for AI Search prompt');
        }
      }
    }
  ...
  // 🔵 NOTE: Don't clear highlights here - let them persist until AI response
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Failed to read orchestrator config:', e);
      ...
      if (forcedProvider) {
        ...
        const PROVIDER_CONFIGS: Record<string, any> = {
          ...
          // ⚠️ OpenAI - GPT-4o Ready (needs user key)
          'openai': {
            provider: 'openai',
          ...
          // ⚠️ Claude - Task-based routing (needs user key)
          'claude': {
            provider: 'claude',
            apiKey: '',  // User sets via Quick Switch panel
            apiBaseUrl: 'https://api.anthropic.com/v1',
          ...
          // ⚠️ Gemini - Task-based routing (needs user key)
          'gemini': {
            provider: 'gemini',
            apiKey: '',  // User sets via Quick Switch panel
            apiBaseUrl: 'https://generativelanguage.googleapis.com/v1beta',
        ...
        if (!baseConfig) {
          ...
          if (savedKey && savedKey.length > 5) {
            ...
            console.log(`⚠️ [FORCED PROVIDER] No API key found for ${forcedProvider}!`);
          }
        }
      ...
      if (useOrchestrator && !forcedProvider) {
        // 🎛️ Use Multi-Provider Orchestrator (only if NOT forced)
        console.log('🎛️ Using Multi-Provider Orchestrator...');
            ...
            try {
              ...
              if (scriptResult.hasScripts) {
                ...
                const summaries = scriptResult.results.map(r => {
                  ...
                  if (r.command === 'ide_rollback' && res) return res.success ? '↩️ Rolled back ' + res.lines_restored + ' lines' : '❌ Rollback failed';
                  return '✅ ' + r.command;
                }).join('\n');
  ...
  document.addEventListener('project-opened', (event: any) => {
    ...
    if (info?.path) {
      ...
      }).catch(e => console.warn('⚠️ Could not read file contents:', e));
    }
  });
  ...
  (window as any).clearAIContext = () => {
    ...
    console.log('🗑️ AI context cleared');
  };
  ...
  setInterval(() => {
    ...
    if (windowPath && windowPath !== lastScannedFolder) {
      ...
      if (lastProject && !windowPath.includes(lastProject.projectName)) {
        console.log('🗑️ Clearing old project context (folder mismatch)');
        (window as any).__lastProject = null;
      }
    ...
    if (!cm.__savePatched) {
      ...
      cm.saveConversations = async function() {
        ...
        if (managerMsgs < localMsgs) {
          console.log(`⚠️ [ConversationFix] BLOCKED save: would lose ${localMsgs - managerMsgs} messages`);
          return;
        }
...
if (localData?.bestConv) {
  ...
  if (isSameConversation && uiCount < localData.bestCount) {
    ...
    console.log(`ℹ️ [ConversationFix] Different conversation active, not overriding`);
  }
}
  }
...
(function initializeAIFileExplorerSystem() {
  ...
  if (existing) {
    ...
    console.log('⚠️ Existing aiFileExplorer is broken, patching...');
  }
...
setTimeout(() => {
  ...
  // If broken, patch it
  if (testEnabled === undefined || testFiles === 0) {
    console.log('⚠️ aiFileExplorer is broken, applying fix...');
...
setTimeout(() => {
  ...
  function tryAttach(retries: number): void {
    ...
    if ((!undoEl || !redoEl) && retries > 0) {
      ...
      console.warn('⚠️ [UndoRedo] Could not find undo/redo UI elements');
    }
  }
