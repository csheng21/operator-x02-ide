...
export function initPiPanel() {
    // Register keyboard shortcut Ctrl+Shift+P (for Pi)
    ...
    if (viewMenu) {
        ...
        menuItem.textContent = '🍓 Raspberry Pi Panel';
        menuItem.dataset.shortcut = 'Ctrl+Shift+P';
        menuItem.addEventListener('click', togglePiPanel);
...
function createPanel() {
    ...
    panel.innerHTML = `
        ...
        ">
            ...
            ${activeConnectionId ? `
                ...
                "></span>
            ` : ''}
            <button onclick="(window as any).showPiHelp()" style="${btnStyle('#6272a4')}">❓</button>
        ...
        ">
            ${['devices','files','terminal','gpio','services','monitor'].map(tab => `
                <button class="pi-tab" data-tab="${tab}" style="${tabStyle(tab === activeTab)}" onclick="(window as any).switchPiTab('${tab}')">
    ...
    // Expose globals
    (window as any).switchPiTab = switchTab;
    (window as any).showPiHelp = showHelp;
    (window as any).piConnectDevice = connectToDevice;
    (window as any).piDisconnect = disconnectDevice;
    (window as any).piScanNetwork = scanNetwork;
    (window as any).piShowAddDevice = showAddDeviceForm;
    (window as any).piNavigate = navigateDirectory;
    (window as any).piOpenFile = openRemoteFile;
    (window as any).piRunCommand = runCommand;
    (window as any).piServiceControl = serviceControl;
    (window as any).piAssignGpioPin = assignGpioPin;
    (window as any).piGenerateGpioCode = generateGpioCode;
    ...
    content.innerHTML = `
        <div style="padding:12px; display:flex; flex-direction:column; gap:10px; height:100%; box-sizing:border-box;">
            ...
            <div style="display:flex; gap:6px; flex-shrink:0;">
                <button onclick="(window as any).piShowAddDevice()" style="${actionBtnStyle('#6272a4')}">+ Add Pi</button>
                <button onclick="(window as any).piScanNetwork()" style="${actionBtnStyle('#44475a')}">🔍 Scan Network</button>
            </div>
        ...
        return `
            <div style="background:#252540; border-radius:8px; padding:10px 12px; border:1px solid #3a3a5c;">
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:${isConnected ? '8px' : '0'};">
                    ...
                    ${isConnected
                        ? `<button onclick="(window as any).piDisconnect('${device.id}')" style="${actionBtnStyle('#ff5555')}">Disconnect</button>`
                        : `<button onclick="(window as any).piConnectDevice('${device.id}')" style="${actionBtnStyle('#50fa7b', '#1a3a1a')}">Connect</button>`
                    }
                </div>
                ${isConnected && info ? `
...
function renderAddDeviceForm(): string {
    return `
        ...
        <div style="display:flex; flex-direction:column; gap:6px;">
            ...
            <select id="pi-f-auth" onchange="(window as any).piToggleAuth(this.value)" style="${inputStyle()}">
            ...
            <div style="display:flex; gap:6px; margin-top:4px;">
                <button onclick="(window as any).piSaveDevice()" style="${actionBtnStyle('#50fa7b', '#1a3a1a')}">💾 Save & Connect</button>
...
function showAddDeviceForm() {
    ...
    (window as any).piToggleAuth = (val: string) => {
    ...
    (window as any).piSaveDevice = async () => {
                ...
                <div style="background:#252540; border-radius:8px; padding:10px; border:1px solid #3a3a5c;">
                    ...
                    ${piDevices.map(d => `
                        <div style="display:flex; align-items:center; gap:8px; padding:5px 0; border-bottom:1px solid #3a3a5c;">
                            ...
                            <button onclick="(window as any).piQuickAdd('${d.ip}')" style="${actionBtnStyle('#6272a4')}">+ Add</button>
                        </div>
                    `).join('')}
                </div>
            `;

            (window as any).piQuickAdd = (ip: string) => {
                showAddDeviceForm();
...
function renderFilesTab(content: HTMLElement) {
    ...
    content.innerHTML = `
        <div style="display:flex; flex-direction:column; height:100%;">
            ...
            <div style="display:flex; align-items:center; gap:6px; padding:8px 12px; background:#1a1a2e; border-bottom:1px solid #3a3a5c; flex-shrink:0;">
                <button onclick="(window as any).piNavigate('..')" style="background:#44475a; border:none; color:#cdd6f4; padding:3px 7px; border-radius:4px; cursor:pointer; font-size:11px;">↑</button>
                <input id="pi-path-input" value="${currentRemotePath}" style="${inputStyle()} flex:1;"
                    onkeydown="if(event.key==='Enter') (window as any).piNavigate(this.value)">
                <button onclick="(window as any).piRefreshFiles()" style="background:#44475a; border:none; color:#cdd6f4; padding:3px 7px; border-radius:4px; cursor:pointer; font-size:11px;">🔄</button>
            </div>
            <!-- File List -->
    ...
    (window as any).piRefreshFiles = () => loadDirectory(currentRemotePath);
    (window as any).piNavigate = (path: string) => {
        if (path === '..') {
    ...
    try {
        ...
        listEl.innerHTML = entries.map(e => `
            <div style="display:flex; align-items:center; gap:6px; padding:5px 6px; border-radius:4px; cursor:pointer; transition:background 0.1s;"
                 ...
                 ondblclick="(window as any).${e.is_dir ? 'piNavigate' : 'piOpenFile'}('${e.path.replace(/'/g, "\\'")}')">
...
async function openRemoteFile(path: string) {
    ...
    try {
        ...
        if ((window as any).openFileInEditor) {
            (window as any).openFileInEditor(path, content, 'pi-remote');
        } else {
    ...
    content.innerHTML = `
        <div style="display:flex; flex-direction:column; height:100%; background:#0d0d1a;">
            ...
            <div style="display:flex; gap:6px; padding:8px; background:#1a1a2e; border-top:1px solid #3a3a5c; flex-shrink:0;">
                ...
                " onkeydown="if(event.key==='Enter') (window as any).piRunCommand()">
                <button onclick="(window as any).piRunCommand()" style="${actionBtnStyle('#50fa7b', '#0d1a0d')}">▶</button>
            </div>
        </div>
    `;
...
function renderGpioTab(content: HTMLElement) {
        ...
        <div style="display:flex; flex-direction:column; height:100%; padding:10px; gap:8px; overflow-y:auto;">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
                ...
                <div style="display:flex; gap:5px;">
                    <button onclick="(window as any).piGenerateGpioCode()" style="${actionBtnStyle('#50fa7b', '#0d1a0d')}">📋 Generate Code</button>
                    <button onclick="(window as any).piClearGpio()" style="${actionBtnStyle('#44475a')}">🗑️ Clear</button>
                </div>
            </div>
                    ...
                    ${GPIO_PINS.map(pin => {
                        ...
                        return `
                            <div style="display:flex; align-items:center; gap:6px; padding:4px 8px; background:${bg}; border:1px solid ${color}; border-radius:5px; cursor:pointer; transition:all 0.1s;"
                                 onclick="(window as any).piAssignGpioPin(${pin.bcm})"
            ...
            <div id="gpio-assign-dialog" style="display:none; background:#252540; border:1px solid #bd93f9; border-radius:8px; padding:12px; flex-shrink:0;">
                ...
                <div style="display:flex; flex-direction:column; gap:6px;">
                    ...
                    <div style="display:flex; gap:6px; margin-top:4px;">
                        <button onclick="(window as any).piSaveGpioPin()" style="${actionBtnStyle('#50fa7b', '#0d1a0d')}">✓ Assign</button>
    ...
    (window as any).piClearGpio = () => { gpioAssignments.clear(); renderTab('gpio'); };
}
...
async function generateGpioCode() {
    ...
    try {
        ...
        // Open in editor
        if ((window as any).openFileInEditor) {
            (window as any).openFileInEditor('gpio_main.py', code, 'new');
        } else {
            console.log(code);
        }
    } catch (err) {
...
function renderServicesTab(content: HTMLElement) {
    ...
    content.innerHTML = `
        <div style="display:flex; flex-direction:column; height:100%; padding:10px; gap:8px;">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
                ...
                <button onclick="(window as any).piLoadServices()" style="${actionBtnStyle('#44475a')}">🔄 Refresh</button>
            </div>
    ...
    (window as any).piLoadServices = loadServices;
    (window as any).piServiceControl = serviceControl;
    loadServices();
}
        ...
        listEl.innerHTML = services.map(svc => `
            <div style="background:#252540; border-radius:6px; padding:8px 10px; margin-bottom:5px; border:1px solid #3a3a5c;">
                ...
                <div style="display:flex; gap:4px;">
                    <button onclick="(window as any).piServiceControl('${svc.name}','start')" style="background:#1a3a1a; border:1px solid #50fa7b; color:#50fa7b; padding:2px 7px; border-radius:3px; cursor:pointer; font-size:10px;">▶</button>
                    <button onclick="(window as any).piServiceControl('${svc.name}','stop')" style="background:#3a1a1a; border:1px solid #ff5555; color:#ff5555; padding:2px 7px; border-radius:3px; cursor:pointer; font-size:10px;">■</button>
                    <button onclick="(window as any).piServiceControl('${svc.name}','restart')" style="background:#3a2a1a; border:1px solid #ffb86c; color:#ffb86c; padding:2px 7px; border-radius:3px; cursor:pointer; font-size:10px;">↺</button>
                    <button onclick="(window as any).piViewServiceLogs('${svc.name}')" style="${actionBtnStyle('#44475a')} font-size:10px; padding:2px 7px;">📋 Logs</button>
                </div>
            </div>
        `).join('');

        (window as any).piViewServiceLogs = async (name: string) => {
            try {
...
function renderMonitorTab(content: HTMLElement) {
    ...
    content.innerHTML = `
        <div style="padding:12px; display:flex; flex-direction:column; gap:8px; height:100%; box-sizing:border-box; overflow-y:auto;">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
                ...
                <button onclick="(window as any).piRefreshMonitor()" style="${actionBtnStyle('#44475a')}">🔄 Refresh</button>
            </div>
    ...
    (window as any).piRefreshMonitor = loadMonitor;
    loadMonitor();
}
...
function renderNoConnection(content: HTMLElement, tab: string) {
    content.innerHTML = `
        <div style="text-align:center; padding:40px 20px; color:#6272a4;">
            ...
            <button onclick="(window as any).switchPiTab('devices')" style="${actionBtnStyle('#6272a4')}">
                Go to Devices
            </button>
        </div>
    `;
...
function showHelp() {
    ...
    `;
    overlay.innerHTML = `
        <div style="background:#1e1e2e; border:1px solid #3a3a5c; border-radius:12px; padding:24px; max-width:500px; width:90%; max-height:80vh; overflow-y:auto; color:#cdd6f4;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
                <div style="font-size:18px; font-weight:700; color:#ff79c6;">🍓 Raspberry Pi Panel Guide</div>
                <button onclick="this.closest('[style*=\"inset:0\"]').remove()" style="background:#44475a; border:none; color:#cdd6f4; padding:4px 10px; border-radius:5px; cursor:pointer; font-size:13px;">✕</button>
            </div>
