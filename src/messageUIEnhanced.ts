...
.ai-message-content .md-list-item::before {
  content: '•';
  position: absolute;
  left: 4px;
  color: #6e7681;
...
export function initializeEnhancedMessageUI(): void {
  ...
  // Enhance any existing code blocks (single pass after DOM settles)
...
function processRawMarkdownInMessages(): void {
  // Find ALL message content divs (not just unprocessed ones for reload scenarios)
  const messageContents = document.querySelectorAll('.ai-message-content, .message-content');
  
  messageContents.forEach((contentDiv) => {
    // Skip if already has enhanced code blocks
    if (contentDiv.querySelector('.enh-code-block') || contentDiv.querySelector('.muf-block')) return;
    
    // Get raw text/HTML content
    let html = contentDiv.innerHTML;
    
    // Check if there are raw markdown code blocks (handle both raw and HTML-encoded)
    const hasCodeBlocks = html.includes('```') || 
                          html.includes('&#96;&#96;&#96;') ||
                          html.includes('&grave;&grave;&grave;');
    
    if (!hasCodeBlocks) {
      // Still process inline elements even without code blocks
      processInlineMarkdown(contentDiv as HTMLElement);
      return;
    }
    
    // Decode HTML entities first
    html = html.replace(/&#96;/g, '`').replace(/&grave;/g, '`');
    ...
    processed = processInlineMarkdownText(processed);
...
/**
 * Process inline markdown elements (bold, code, headers, etc.)
 */
function processInlineMarkdown(element: HTMLElement): void {
  // Skip if already processed
  if (element.classList.contains('inline-md-processed')) return;
  element.classList.add('inline-md-processed');
  
  element.innerHTML = processInlineMarkdownText(element.innerHTML);
}

/**
 * Process inline markdown in text
 */
function processInlineMarkdownText(text: string): string {
  let processed = text;
...
if (typeof window !== 'undefined') {
  ...
  (window as any).enhancedMessageUI = {
    ...
    processMarkdown: processRawMarkdownInMessages,
  };
}
