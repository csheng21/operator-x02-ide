...
async function executeCommand(cmd: string) {
  ...
  try {
    ...
    const out = typeof result === 'string' ? result : (result as any)?.output ?? (result as any)?.stdout ?? (result as any)?.stderr ?? JSON.stringify(result ?? '');
    const err = typeof result === 'string' ? '' : ((result as any)?.stderr ?? '');
...
export function openJetsonTerminal() {
  ...
  // Buttons
  panel.querySelector('#jterm-close')!.addEventListener('click', () => { if(window.__jt_intervals){window.__jt_intervals.forEach((id:any)=>clearInterval(id));window.__jt_intervals=[];} panel.style.display='none'; });
