...
async function runProfiler(remoteFile: string, remoteDir: string) {
  ...
  try {
    ...
    const out = typeof result === 'string' ? result : (result as any)?.output ?? (result as any)?.stdout ?? JSON.stringify(result ?? '');
...
export function openJetsonDevTools() {
  ...
  panel.querySelector('#jdt-close')?.addEventListener('click', () => {
    stopLiveLog(); if(window.__jt_intervals){window.__jt_intervals.forEach((id:any)=>clearInterval(id));window.__jt_intervals=[];} panel.style.display='none';
  });
