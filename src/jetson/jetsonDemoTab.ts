...
import { mountConnectionPanel, mountDeployPanel } from './jetsonIntegration';
...
function wire(): void {
  ...
  qb('jt-mon-go')?.addEventListener('click', async () => {
...
function startPoll(): void {
  ...
  monIv = setInterval(async () => {
    if (!root || !conn || polling) return;
    polling = true;
    try {
      const raw = await invoke('jetson_tegrastats_snapshot');
...
function renderStats(s: TegraStats): void {
  if (!root) return;
  (window as any).__jetsonLastStats = s; // for PERF panel
...
// ============================================================
// FEATURE BAR PANEL IMPLEMENTATIONS
// ============================================================

function showFeaturePanel(id: string, title: string, buildFn: (body: HTMLElement) => void): void {
  document.getElementById(id)?.remove();
  const overlay = document.createElement('div');
  overlay.id = id;
  overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:99998;background:rgba(0,0,0,0.65);display:flex;align-items:center;justify-content:center;';
  const panel = document.createElement('div');
  panel.style.cssText = 'background:#0d1117;border:1px solid #30363d;border-radius:12px;width:700px;max-width:92vw;max-height:82vh;display:flex;flex-direction:column;font-family:JetBrains Mono,monospace;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.8);';
  const hdr = document.createElement('div');
  hdr.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid #30363d;background:#161b22;flex-shrink:0;';
  hdr.innerHTML = '<span style="color:#76b900;font-weight:700;font-size:13px;">' + title + '</span><button id="' + id + '-close" style="background:none;border:none;color:#8b949e;font-size:18px;cursor:pointer;padding:0 4px;line-height:1;">&#x2715;</button>';
  const body = document.createElement('div');
  body.style.cssText = 'flex:1;overflow:auto;padding:16px;';
  panel.appendChild(hdr); panel.appendChild(body);
  overlay.appendChild(panel); document.body.appendChild(overlay);
  document.getElementById(id + '-close')?.addEventListener('click', () => overlay.remove());
  overlay.addEventListener('click', (e: MouseEvent) => { if (e.target === overlay) overlay.remove(); });
  buildFn(body);
}

// 1. TERMINAL
function openJetsonTerminal(): void {
  if (!conn) { alert('Connect to Jetson first'); return; }
  showFeaturePanel('jt-panel-term', '>_ Remote Terminal', (body: HTMLElement) => {
    body.innerHTML = '<div id="jt-term-out" style="background:#010409;border-radius:6px;padding:10px;height:300px;overflow-y:auto;font-size:11px;color:#e6edf3;white-space:pre-wrap;margin-bottom:10px;"></div><div style="display:flex;gap:8px;align-items:center;"><span style="color:#76b900;font-size:12px;">$</span><input id="jt-term-in" style="flex:1;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#e6edf3;font-family:inherit;font-size:12px;" placeholder="Enter command..." /><button id="jt-term-run" style="background:#76b900;border:none;border-radius:4px;padding:6px 14px;color:#000;font-weight:700;cursor:pointer;font-size:12px;">Run</button></div>';
    const out = document.getElementById('jt-term-out')!;
    const inp = document.getElementById('jt-term-in') as HTMLInputElement;
    out.innerHTML = '<span style="color:#8b949e">Connected to Jetson. Type commands below.\n\n</span>';
    const run = async () => {
      const cmd = inp.value.trim(); if (!cmd) return;
      out.innerHTML += '<span style="color:#76b900">$ ' + cmd + '</span>\n';
      inp.value = ''; inp.disabled = true;
      try {
        const result = await invoke('jetson_execute', { command: cmd }) as string;
        out.innerHTML += (result || '(no output)') + '\n\n';
      } catch(e: unknown) { out.innerHTML += '<span style="color:#f85149">Error: ' + String(e) + '</span>\n\n'; }
      inp.disabled = false; inp.focus(); out.scrollTop = out.scrollHeight;
    };
    document.getElementById('jt-term-run')?.addEventListener('click', run);
    inp.addEventListener('keydown', (e: KeyboardEvent) => { if (e.key === 'Enter') run(); });
    inp.focus();
  });
}

// 2. FILES
function openJetsonFileBrowser(): void {
  if (!conn) { alert('Connect to Jetson first'); return; }
  showFeaturePanel('jt-panel-files', '[+] File Browser', (body: HTMLElement) => {
    body.innerHTML = '<div style="display:flex;gap:8px;margin-bottom:10px;"><input id="jt-files-path" value="/home" style="flex:1;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:5px 10px;color:#e6edf3;font-family:inherit;font-size:12px;" /><button id="jt-files-go" style="background:#76b900;border:none;border-radius:4px;padding:5px 12px;color:#000;font-weight:700;cursor:pointer;font-size:12px;">Go</button><button id="jt-files-up" style="background:#161b22;border:1px solid #30363d;border-radius:4px;padding:5px 10px;color:#8b949e;cursor:pointer;font-size:11px;">&#x2191; Up</button></div><div id="jt-files-list" style="background:#010409;border-radius:6px;padding:10px;min-height:200px;max-height:340px;overflow-y:auto;font-size:11px;"></div><div style="margin-top:8px;color:#8b949e;font-size:10px;">Click a directory to navigate.</div>';
    const ls = async (dir: string) => {
      const listEl = document.getElementById('jt-files-list')!;
      listEl.innerHTML = '<span style="color:#8b949e">Loading...</span>';
      (document.getElementById('jt-files-path') as HTMLInputElement).value = dir;
      try {
        const result = await invoke('jetson_execute', { command: 'ls -la "' + dir + '" 2>&1' }) as string;
        const lines = result.split('\n');
        listEl.innerHTML = lines.map((l: string) => {
          if (!l.trim() || l.startsWith('total')) return '<div style="color:#8b949e;font-size:10px;">' + l + '</div>';
          const isDir = l.startsWith('d');
          const parts = l.split(/\s+/);
          const name = parts.slice(8).join(' ');
          if (!name) return '';
          const color = isDir ? '#58a6ff' : '#e6edf3';
          const attr = isDir ? ' data-dir="' + dir.replace(/\/+$/,'') + '/' + name + '"' : '';
          return '<div' + attr + ' style="color:' + color + ';padding:3px 6px;border-radius:3px;cursor:' + (isDir?'pointer':'default') + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="' + l + '">' + l + '</div>';
        }).join('');
        listEl.querySelectorAll('[data-dir]').forEach((el: Element) => {
          el.addEventListener('click', () => ls((el as HTMLElement).dataset.dir!));
        });
      } catch(e: unknown) { listEl.innerHTML = '<span style="color:#f85149">Error: ' + String(e) + '</span>'; }
    };
    document.getElementById('jt-files-go')?.addEventListener('click', () => ls((document.getElementById('jt-files-path') as HTMLInputElement).value.trim()));
    document.getElementById('jt-files-up')?.addEventListener('click', () => {
      const p = (document.getElementById('jt-files-path') as HTMLInputElement).value;
      ls(p.replace(/\/[^/]+\/?$/, '') || '/');
    });
    (document.getElementById('jt-files-path') as HTMLInputElement)?.addEventListener('keydown', (e: KeyboardEvent) => { if (e.key === 'Enter') document.getElementById('jt-files-go')?.click(); });
    ls('/home');
  });
}

// 3. PERF GRAPH
function openJetsonPerfGraph(): void {
  showFeaturePanel('jt-panel-perf', '/ Performance Graph', (body: HTMLElement) => {
    const metrics = ['GPU %', 'CPU %', 'RAM %'];
    const colors = ['#76b900', '#58a6ff', '#d29922'];
    const history: number[][] = [[], [], []];
    const MAX = 60; const W = 560; const H = 100;
    body.innerHTML = metrics.map((_m, i) => '<div style="margin-bottom:14px;"><div style="color:' + colors[i] + ';font-size:11px;margin-bottom:4px;">' + metrics[i] + ' <span id="jt-pv' + i + '" style="color:#e6edf3;font-weight:700;"></span></div><canvas id="jt-pc' + i + '" width="' + W + '" height="' + H + '" style="width:100%;height:' + H + 'px;background:#010409;border-radius:6px;display:block;"></canvas></div>').join('') + '<div style="color:#8b949e;font-size:10px;margin-top:4px;">Live from tegrastats stream. Start monitoring on the main panel first.</div>';
    const draw = () => {
      metrics.forEach((_m, i) => {
        const canvas = document.getElementById('jt-pc' + i) as HTMLCanvasElement;
        if (!canvas) return;
        const ctx = canvas.getContext('2d')!;
        ctx.clearRect(0, 0, W, H);
        ctx.strokeStyle = colors[i] + '30';
        [25, 50, 75].forEach(y => { ctx.beginPath(); ctx.moveTo(0, H * y / 100); ctx.lineTo(W, H * y / 100); ctx.stroke(); });
        const data = history[i]; if (data.length < 2) return;
        ctx.strokeStyle = colors[i]; ctx.lineWidth = 2; ctx.beginPath();
        data.forEach((v, x) => { const px = x * W / MAX; const py = H - (v / 100 * H); x === 0 ? ctx.moveTo(px, py) : ctx.lineTo(px, py); });
        ctx.stroke();
        ctx.fillStyle = colors[i] + '18';
        ctx.lineTo((data.length - 1) * W / MAX, H); ctx.lineTo(0, H); ctx.fill();
      });
    };
    const iv = setInterval(() => {
      const s = (window as any).__jetsonLastStats as TegraStats | undefined;
      if (!s) return;
      const vals = [
        Math.min(100, Math.max(0, s.gpu_usage || 0)),
        Math.min(100, Math.max(0, s.cpu_usage ? s.cpu_usage.reduce((a: number, b: number) => a + b, 0) / s.cpu_usage.length : 0)),
        Math.min(100, Math.max(0, s.ram_total_mb ? (s.ram_used_mb / s.ram_total_mb) * 100 : 0))
      ];
      vals.forEach((v, i) => { history[i].push(v); if (history[i].length > MAX) history[i].shift(); const el = document.getElementById('jt-pv' + i); if (el) el.textContent = v.toFixed(1) + '%'; });
      draw();
    }, 1000);
    const cleanup = () => clearInterval(iv);
    document.getElementById('jt-panel-perf-close')?.addEventListener('click', cleanup);
    document.getElementById('jt-panel-perf')?.addEventListener('click', (e: Event) => { if ((e.target as HTMLElement).id === 'jt-panel-perf') cleanup(); });
  });
}

// 4. POWER MANAGER
function openJetsonPowerManager(): void {
  if (!conn) { alert('Connect to Jetson first'); return; }
  showFeaturePanel('jt-panel-power', '~ Power & AI Tools', (body: HTMLElement) => {
    const profiles = [['MAXN (Full Power)', '0', '#76b900'], ['15W', '1', '#d29922'], ['10W', '2', '#d29922'], ['7W (Eco)', '3', '#f85149']];
    body.innerHTML = '<div style="margin-bottom:18px;"><div style="color:#8b949e;font-size:10px;letter-spacing:1px;margin-bottom:10px;">NVPMODEL POWER PROFILE</div><div style="display:flex;gap:8px;flex-wrap:wrap;" id="jt-pwr-btns">' + profiles.map(([label, mode, color]) => '<button data-mode="' + mode + '" style="background:' + color + '15;border:1px solid ' + color + ';border-radius:6px;padding:8px 16px;color:' + color + ';font-family:inherit;font-size:11px;cursor:pointer;font-weight:600;">' + label + '</button>').join('') + '</div><div id="jt-pwr-status" style="margin-top:10px;font-size:11px;color:#8b949e;min-height:18px;"></div></div><div style="border-top:1px solid #30363d;padding-top:16px;"><div style="color:#8b949e;font-size:10px;letter-spacing:1px;margin-bottom:10px;">TENSORRT CONVERSION</div><input id="jt-trt-in" placeholder="Remote ONNX path  e.g. /home/user/model.onnx" style="width:100%;box-sizing:border-box;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:7px 10px;color:#e6edf3;font-family:inherit;font-size:11px;margin-bottom:8px;" /><input id="jt-trt-out" placeholder="Output .engine path (optional)" style="width:100%;box-sizing:border-box;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:7px 10px;color:#e6edf3;font-family:inherit;font-size:11px;margin-bottom:10px;" /><button id="jt-trt-run" style="background:#76b900;border:none;border-radius:4px;padding:7px 18px;color:#000;font-weight:700;cursor:pointer;font-size:12px;">&#x25B6; Convert with TensorRT</button><div id="jt-trt-log" style="margin-top:10px;font-size:10px;background:#010409;border-radius:4px;padding:8px;min-height:36px;white-space:pre-wrap;display:none;color:#8b949e;"></div></div>';
    document.getElementById('jt-pwr-btns')?.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', async () => {
        const mode = (btn as HTMLElement).dataset.mode;
        const s = document.getElementById('jt-pwr-status')!;
        s.style.color = '#8b949e'; s.textContent = 'Applying power mode ' + mode + '...';
        try {
          await invoke('jetson_execute', { command: 'sudo nvpmodel -m ' + mode + ' 2>&1' });
          const r = await invoke('jetson_execute', { command: 'nvpmodel -q 2>&1' }) as string;
          s.style.color = '#76b900'; s.textContent = '✓ ' + (r.split('\n')[0] || 'Mode ' + mode + ' applied');
        } catch(e: unknown) { s.style.color = '#f85149'; s.textContent = 'Error: ' + String(e); }
      });
    });
    document.getElementById('jt-trt-run')?.addEventListener('click', async () => {
      const inp = (document.getElementById('jt-trt-in') as HTMLInputElement).value.trim();
      const out = (document.getElementById('jt-trt-out') as HTMLInputElement).value.trim() || inp.replace('.onnx', '.engine');
      const log = document.getElementById('jt-trt-log')!;
      if (!inp) { alert('Enter ONNX model path'); return; }
      log.style.display = 'block'; log.style.color = '#8b949e'; log.textContent = 'Running TensorRT conversion — this may take 1–3 minutes...';
      try {
        const result = await invoke('jetson_execute', { command: '/usr/src/tensorrt/bin/trtexec --onnx="' + inp + '" --saveEngine="' + out + '" 2>&1 | tail -30' }) as string;
        log.style.color = '#76b900'; log.textContent = result;
      } catch(e: unknown) { log.style.color = '#f85149'; log.textContent = 'Error: ' + String(e); }
    });
  });
}

// 5. MULTI-DEVICE
function openJetsonMultiDevice(): void {
  showFeaturePanel('jt-panel-multi', ':: Multi-Device Manager', (body: HTMLElement) => {
    const devices: {host: string; user: string}[] = [];
    body.innerHTML = '<div style="color:#8b949e;font-size:11px;margin-bottom:14px;">Add multiple Jetson boards and broadcast commands to all simultaneously.</div><div id="jt-multi-list" style="min-height:40px;margin-bottom:12px;display:flex;flex-direction:column;gap:6px;"></div><div style="display:flex;gap:6px;margin-bottom:14px;"><input id="jt-ml-host" placeholder="IP:port" style="flex:2;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#e6edf3;font-family:inherit;font-size:11px;" /><input id="jt-ml-user" placeholder="username" style="flex:1;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#e6edf3;font-family:inherit;font-size:11px;" /><button id="jt-ml-add" style="background:#76b900;border:none;border-radius:4px;padding:6px 12px;color:#000;font-weight:700;cursor:pointer;font-size:11px;">Add</button></div><div style="border-top:1px solid #30363d;padding-top:12px;"><input id="jt-ml-cmd" placeholder="Command to broadcast to all devices..." style="width:100%;box-sizing:border-box;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#e6edf3;font-family:inherit;font-size:11px;margin-bottom:8px;" /><button id="jt-ml-run" style="background:#161b22;border:1px solid #76b900;border-radius:4px;padding:6px 14px;color:#76b900;font-family:inherit;font-size:11px;cursor:pointer;">&#x25BA; Run on All</button><div id="jt-ml-res" style="margin-top:8px;font-size:10px;color:#8b949e;white-space:pre-wrap;max-height:120px;overflow-y:auto;"></div></div>';
    const renderList = () => {
      const el = document.getElementById('jt-multi-list')!;
      el.innerHTML = devices.length ? devices.map((d, i) => '<div style="display:flex;align-items:center;gap:10px;background:#161b22;border-radius:6px;padding:7px 12px;"><span style="color:#76b900;font-size:9px;">&#x25CF;</span><span style="flex:1;color:#e6edf3;font-size:11px;">' + d.host + '</span><span style="color:#8b949e;font-size:10px;">' + d.user + '</span><button data-rm="' + i + '" style="background:none;border:none;color:#f85149;cursor:pointer;font-size:13px;padding:0 2px;">&#x2715;</button></div>').join('') : '<div style="color:#8b949e;font-size:11px;">No devices added.</div>';
      el.querySelectorAll('[data-rm]').forEach(btn => btn.addEventListener('click', () => { devices.splice(parseInt((btn as HTMLElement).dataset.rm!), 1); renderList(); }));
    };
    document.getElementById('jt-ml-add')?.addEventListener('click', () => {
      const host = (document.getElementById('jt-ml-host') as HTMLInputElement).value.trim();
      const user = (document.getElementById('jt-ml-user') as HTMLInputElement).value.trim() || 'jetson';
      if (!host) return;
      devices.push({host, user}); renderList();
      (document.getElementById('jt-ml-host') as HTMLInputElement).value = '';
    });
    document.getElementById('jt-ml-run')?.addEventListener('click', () => {
      const cmd = (document.getElementById('jt-ml-cmd') as HTMLInputElement).value.trim();
      const res = document.getElementById('jt-ml-res')!;
      if (!cmd || !devices.length) { res.textContent = 'Add devices and enter a command.'; return; }
      res.style.color = '#d29922'; res.textContent = 'Broadcasting "' + cmd + '" to ' + devices.length + ' device(s)...\n\nNote: Multi-device parallel execution coming in v1.6. Currently runs via active connection only.';
    });
    renderList();
  });
}
...
// 6. DEVTOOLS
function openJetsonDevTools(): void {
  if (!conn) { alert('Connect to Jetson first'); return; }
  showFeaturePanel('jt-panel-dev', '{} Dev Tools', (body: HTMLElement) => {
    const quickCmds: [string, string][] = [
      ['System Logs', 'journalctl -n 40 --no-pager 2>&1'],
      ['GPU Info', 'cat /proc/device-tree/model 2>/dev/null && cat /etc/nv_tegra_release 2>/dev/null | head -5'],
      ['CUDA Devices', 'nvidia-smi 2>/dev/null || echo "nvidia-smi not found"'],
      ['Processes', 'ps aux --sort=-%cpu 2>/dev/null | head -20'],
      ['Disk Usage', 'df -h 2>&1'],
      ['Network', 'ip addr show 2>&1'],
      ['Memory', 'free -h 2>&1 && cat /proc/meminfo 2>&1 | head -10'],
      ['Jetson Clocks', 'cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq 2>/dev/null | head -6']
    ];
    body.innerHTML = '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;">' + quickCmds.map(([label]) => '<button data-cmd-label="' + label + '" style="background:#161b22;border:1px solid #30363d;border-radius:4px;padding:4px 10px;color:#8b949e;font-family:inherit;font-size:10px;cursor:pointer;">' + label + '</button>').join('') + '</div><div style="display:flex;gap:8px;margin-bottom:10px;"><input id="jt-dev-in" placeholder="Custom command..." style="flex:1;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#e6edf3;font-family:inherit;font-size:11px;" /><button id="jt-dev-run" style="background:#76b900;border:none;border-radius:4px;padding:6px 14px;color:#000;font-weight:700;cursor:pointer;font-size:11px;">Run</button><button id="jt-dev-clr" style="background:#161b22;border:1px solid #30363d;border-radius:4px;padding:6px 10px;color:#8b949e;cursor:pointer;font-size:11px;">Clear</button></div><div id="jt-dev-out" style="background:#010409;border-radius:6px;padding:10px;height:260px;overflow-y:auto;font-size:10px;color:#e6edf3;white-space:pre-wrap;"></div>';
    const out = document.getElementById('jt-dev-out')!;
    ...
    const cmdMap = Object.fromEntries(quickCmds);
    body.querySelectorAll('[data-cmd-label]').forEach(btn => btn.addEventListener('click', () => runCmd(cmdMap[(btn as HTMLElement).dataset.cmdLabel!])));
    document.getElementById('jt-dev-run')?.addEventListener('click', () => { const v = (document.getElementById('jt-dev-in') as HTMLInputElement).value.trim(); if (v) runCmd(v); });
    document.getElementById('jt-dev-clr')?.addEventListener('click', () => { out.innerHTML = ''; });
    (document.getElementById('jt-dev-in') as HTMLInputElement)?.addEventListener('keydown', (e: KeyboardEvent) => { if (e.key === 'Enter') document.getElementById('jt-dev-run')?.click(); });
    runCmd('cat /proc/device-tree/model 2>/dev/null && uname -a 2>&1');
  });
}
