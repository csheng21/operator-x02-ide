...
export class SurgicalBackupManager {
  ...
  private _statsCacheTTL: number = 60000; // X02-lagfix: raised from 5s to 60s — prevents stats spam on every write_file
  ...
  async getStats(): Promise<BackupStats> {
    ...
    // X02-lagfix: log only at debug level — was firing on every write_file
    console.debug('📊 [BackupManager] Getting stats (cache miss)...');
