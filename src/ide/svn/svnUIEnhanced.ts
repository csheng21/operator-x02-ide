...
private renderAIAnalytics(analytics: AIFileAnalytics): void {
    ...
    /**
     * ✨ Update working copy (if not already present)
     */
    private async updateWorkingCopy(): Promise<void> {
        try {
            console.log('⬇️ Updating working copy...');
            const result = await svnManager.update(this.currentPath);
            this.showNotification('✅ Working copy updated', 'success');
            await this.refresh();
        } catch (error) {
            console.error('Update failed:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.showNotification('Failed to update: ' + errorMessage, 'error');
        }
    }

    /**
     * ✨ Cleanup working copy (if not already present)
     */
    private async cleanupWorkingCopy(): Promise<void> {
        try {
            console.log('🧹 Cleaning up working copy...');
            const result = await svnManager.cleanup(this.currentPath);
            this.showNotification('✅ Cleanup complete', 'success');
            await this.refresh();
        } catch (error) {
            console.error('Cleanup failed:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.showNotification('Failed to cleanup: ' + errorMessage, 'error');
        }
    }
